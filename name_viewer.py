...
class NameViewerApp:
    def __init__(self, root, search_query=None):
        self.root = root
    ...
    def populate_tree(self):
        for pacient in self.parsed_data:
            if self.search_query is None or pacient['Nume'].lower() == self.search_query.lower():
...
if __name__ == "__main__":
    root = tk.Tk()
    search_query = None  # Daca nu s-a introdus niciun text pentru cautare
    app = NameViewerApp(root, search_query)
    root.mainloop()
